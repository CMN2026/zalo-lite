...
export default function ChatView() {
  ...
  const activeMock =
    activeChat.source === "mock"
      ? mockConversations.find((c) => c.id === activeChat.id) ?? mockConversations[0]
      : null;
        ...
        <div className="flex-1 flex flex-col bg-slate-50 relative">
          <div className="h-16 bg-white border-b border-slate-200 px-6 flex items-center justify-between shadow-sm z-10">
            <div className="flex items-center gap-3">
              <img src={activeMock.avatar} alt={activeMock.name} className="w-9 h-9 rounded-full object-cover" />
              <div>
                <h2 className="text-sm font-semibold">{activeMock.name}</h2>
                <div className="flex items-center gap-1.5 text-xs text-slate-500">
                  <span className={`w-2 h-2 rounded-full ${activeMock.online ? 'bg-green-500' : 'bg-slate-300'}`}></span>
      ...
      {activeMock && (
        <div className="w-80 border-l border-slate-200 bg-white flex flex-col overflow-y-auto">
          <div className="flex flex-col items-center py-8 border-b border-slate-100">
            <img src={activeMock.avatar} alt={activeMock.name} className="w-20 h-20 rounded-full object-cover border-4 border-white shadow-sm mb-3" />
          ...
          <div className="p-6 border-b border-slate-100 space-y-3">
            <div className="flex justify-between text-sm"><span className="text-slate-500">Subject:</span><span className="font-medium text-right">{activeMock.subject}</span></div>
            <div className="flex justify-between text-sm"><span className="text-slate-500">Category:</span><span className="font-medium text-right">{activeMock.category}</span></div>
          </div>
        </div>
      )}
